...
class EditEvent extends StatefulWidget {
  ...
  final int id;
  const EditEvent({super.key, required this.event, required this.id});
...
class _EditEventState extends State<EditEvent> {
  ...
  void initState() {
    super.initState();
    // Initialize the controllers with existing event data
  ...
  Future<void> _editEvent() async {
    if (_formKey.currentState!.validate()) {
      ...
      final uri = Uri.parse('$kBaseurl/api/events/${widget.id}/'); // Use event id for the PUT request
      ...
      // If image is selected, include it in the request
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Event "$title" updated successfully!')),
        );
